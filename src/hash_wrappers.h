...
namespace hashes {
    ...
    private:
        [[nodiscard]] virtual uint32_t Hash(const std::string& str) const = 0;
    };
    ...
    private:
        [[nodiscard]] virtual uint64_t Hash(const std::string& str) const = 0;
    };
...
//----- Bernstein's hash djb2 ------
    ...
    private:
        [[nodiscard]] uint32_t Hash(const std::string &str) const override;
    };

    class [[maybe_unused]] DJB2Hash64Wrapper : public BaseHash64Wrapper {
    private:
        [[nodiscard]] uint64_t Hash(const std::string &str) const override;
    };
...
//----- Rolling Hash (BuzHash) -----
    ...
    private:
        uint32_t Hash(const std::string &str) const override;

        mutable CyclicHash<uint32_t, unsigned char> hash_function_{WORD_SIZE, 32};
    };
    ...
    private:
        uint64_t Hash(const std::string &str) const override;
